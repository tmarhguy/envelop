 ...
 *
 * Provenance: this is a vendored snapshot of web/js/tomato-cpu.js from the
 * sibling Tomato source repository. The adjacent tomato-os.bin is copied from
 * Tomato's web/data/tomato-os.bin, generated by tools/build_web_image.py from
 * OS, microcode, and display sources. Those Tomato sources, not this copy, are
 * authoritative; update the module and image together.
 *
 ...
 * timing, no FPGA resources. The source repository's
 * web/tests/tomato-emu.mjs compares its current emulator and generated image
 * with RTL/program expectations; Envelop's focused tests cover this snapshot's
...
// Microcode plane order in the image (matches Tomato's tools/build_web_image.py).
...
// ACI-mailbox radio, ported from Tomato's tools/virtual_tomato.cpp at the
// mailbox boundary (not electrical SPI).
class Radio {
...
export class Tomato {
  ...
  stepOne() {
    ...
    if ((memRd && isKb && !isBle && (addr & 3) === 0) || op === OP_IN) this.kbReady = false;
